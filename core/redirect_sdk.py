...
class RedirectClient:
    ...
    def fetch_redirect(
        ...
        if r.status_code == 422:
            ...
            if auth_domain not in r_id:
                r_id = f"{auth_domain}/{r_id}"
            elif subdomain is not None and auth_domain not in r_id:
                r_id = f"{subdomain}.{auth_domain}/{r_id}"
    ...
    def del_redirect(self, id_or_path: str, sub_domain: str = None) -> int:
        ...
        try:
            ...
            found = False
            self.get_redirects()
            if auth_domain not in id_or_path and sub_domain is None:
                id_or_path = f"{auth_domain}/{id_or_path}"
            elif sub_domain is not None and auth_domain not in id_or_path:
                id_or_path = f"{sub_domain}.{auth_domain}/{id_or_path}"
